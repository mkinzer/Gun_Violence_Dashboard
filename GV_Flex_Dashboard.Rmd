...
dateInput(inputId = "enddate2", label = "Select End Date", value = "2019-12-31",min = "2013-01-01", format = "yyyy-mm-dd")
...
renderHighchart({
  ...
  hchart(philly.gv_perc, "bar", hcaes(x = Variable, y = Percentage)) %>% 
    ...
    hc_title(text = "Demographic Percentages in Shooting Incidents") %>% 
    hc_legend(enabled = F)
...
The **United States GV** tab shows nation-wide data scraped from both the **[Gun Violence Archive](https://www.gunviolencearchive.org/) (GVA)** and the **[Mass Shooting Tracker](https://www.massshootingtracker.org/) (MST)** and combines them to ensure as many incidents are tracked and verified as possible.** 
...
The **Philadelphia PA GV** tab scrapes data from **[Open Data Philly](https://www.opendataphilly.org/dataset/shooting-victims)** which is self-branded as "the official open data repository for the City." This tab does not look into "mass shooting" events and instead looks at all reported shooting incidents. 
